...
//   const addToCart    = (product:any, qty=1) => setCart(prev=>({...prev,[product._id]:{product,qty:(prev[product._id]?.qty||0)+qty}}));
//   const updateQty    = (id:string, delta:number) => setCart(prev=>{ const next=(prev[id]?.qty||0)+delta; if(next<=0){const u={...prev};delete u[id];return u;} return{...prev,[id]:{...prev[id],qty:next}}; });
//   const removeFromCart = (id:string) => setCart(prev=>{const u={...prev};delete u[id];return u;});
//   const addBulkToCart  = () => { filtered.filter(p=>selected.has(p._id)).forEach(p=>addToCart(p,1)); setSelected(new Set()); setBulkMode(false); setCartOpen(true); };
...
//       {/* ── Page Header
//           No back button here — the navbar in ConsumerLayout handles it.
//           No "Back to Consumer Dashboard" link here either — that was the duplicate.
//       ── */}
//       <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
//         <div>
//           <h1 style={{ fontSize:22, fontWeight:800, color:COLOR.text, margin:0, fontFamily:"'Playfair Display',serif" }}>
//             Marketplace
//           </h1>
//           <p style={{ fontSize:13, color:COLOR.muted, marginTop:3 }}>
//             {filtered.length} products from verified farmers
...
//             {bulkMode?"Exit Select":"Bulk Select"}
//           </button>
//           <button className="btn-h" onClick={()=>setCartOpen(true)}
//             style={{ position:"relative", background:"linear-gradient(135deg,#1a3a2a,#2d5a3d)", color:"#fff", border:"none", borderRadius:10, padding:"9px 18px", fontSize:13, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:7 }}>
//             <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
//             Cart
//             {cartCount>0&&<span style={{ background:"#ef4444", color:"#fff", borderRadius:"50%", width:18, height:18, fontSize:10, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", position:"absolute", top:-7, right:-7 }}>{cartCount}</span>}
//           </button>
...
//           </div>
//           <div style={{ display:"flex", gap:8 }}>
//             <button className="btn-h" onClick={()=>setCartOpen(true)} style={{ background:"none", border:"1.5px solid #22c55e", color:"#166634", borderRadius:8, padding:"5px 14px", fontSize:12, fontWeight:700, cursor:"pointer" }}>View Cart</button>
//             <button className="btn-h" onClick={()=>{setCartOpen(false);setCheckoutOpen(true);}} style={{ background:"linear-gradient(135deg,#1a3a2a,#22c55e)", color:"#fff", border:"none", borderRadius:8, padding:"5px 14px", fontSize:12, fontWeight:700, cursor:"pointer" }}>Checkout →</button>
//           </div>
//         </div>
//       )}
...
//       {loading?(
//         <div style={{ textAlign:"center", padding:"80px", color:COLOR.muted }}>
//           <div style={{ fontSize:14, fontWeight:600 }}>Loading fresh produce…</div>
//         </div>
//       ):filtered.length===0?(
//         <div style={{ textAlign:"center", padding:"80px", color:"#9ca3af" }}>
//           <div style={{ fontSize:48, marginBottom:12 }}>🛒</div>
...
//         <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:18 }}>
//           {filtered.map((p:any,idx:number)=>{
//             const inC=!!cart[p._id]; const qty=cart[p._id]?.qty||0;
//             const isSel=selected.has(p._id); const tags=NUTRIENT_TAGS[p.cropName]||[];
//             return (
//               <div key={p._id}
//                 className={`product-card card-h fu ${isSel?"sel-card":""}`}
//                 style={{ background:"#fff", borderRadius:16, border:`1.5px solid ${inC?"#bbf7d0":isSel?"#22c55e":"#eeebe4"}`, overflow:"hidden", boxShadow:inC?"0 4px 18px rgba(34,197,94,.12)":"0 2px 8px rgba(0,0,0,.04)", animationDelay:`${idx*0.03}s` }}
//                 onClick={()=>bulkMode?toggleSelect(p._id):undefined}>
//                 <div style={{ height:160, position:"relative", overflow:"hidden" }}>
//                   <ProductImg product={p} h={160}/>
//                   {bulkMode&&(
...
//                   )}
//                   {inC&&!bulkMode&&(
//                     <div style={{ position:"absolute", top:10, left:10, background:"#22c55e", color:"#fff", borderRadius:99, fontSize:10, fontWeight:700, padding:"3px 9px", zIndex:2 }}>In Cart ({qty})</div>
//                   )}
//                   {p.type==="Organic"&&(
//                     <div style={{ position:"absolute", top:10, right:10, background:"#f0fdf4", border:"1px solid #bbf7d0", color:"#166534", fontSize:10, fontWeight:700, padding:"3px 9px", borderRadius:99, zIndex:2 }}>Organic</div>
//                   )}
//                 </div>
//                 <div style={{ padding:"14px 16px" }}>
//                   <div style={{ fontSize:15, fontWeight:700, color:COLOR.text, marginBottom:2 }}>{p.cropName}</div>
...
//                   {tags.length>0&&(
//                     <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:8 }}>
//                       {tags.map(t=><span key={t} style={{ background:"#f0fdf4", color:"#166534", fontSize:9, fontWeight:700, padding:"2px 7px", borderRadius:99, border:"1px solid #bbf7d0" }}>{t}</span>)}
//                     </div>
//                   )}
//                   {p.trustScore>0&&<div style={{ fontSize:11, color:"#f59e0b", marginBottom:8 }}>{"★".repeat(Math.round(p.trustScore/20))} {(p.trustScore/20).toFixed(1)}</div>}
//                   <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
//                     <div>
//                       <div style={{ fontSize:16, fontWeight:800, color:COLOR.text }}>Rs.{p.price}<span style={{ fontSize:11, fontWeight:400, color:COLOR.muted }}>/kg</span></div>
//                       <div style={{ fontSize:10, color:COLOR.muted }}>{p.stock}kg available</div>
...
//               ):cartItems.map(({product:p,qty})=>(
//                 <div key={p._id} style={{ display:"flex", alignItems:"center", gap:13, padding:"13px 0", borderBottom:"1px solid #f8f5f0" }}>
//                   <div style={{ width:52, height:52, borderRadius:11, overflow:"hidden", flexShrink:0 }}><ProductImg product={p} h={52}/></div>
//                   <div style={{ flex:1, minWidth:0 }}>
//                     <div style={{ fontSize:14, fontWeight:700, color:COLOR.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{p.cropName}</div>
//                     <div style={{ fontSize:12, color:COLOR.muted }}>Rs.{p.price}/kg</div>
//                     <div style={{ fontSize:13, fontWeight:700, color:"#22c55e" }}>Rs.{(qty*p.price).toLocaleString()}</div>
...
//                     <button onClick={()=>updateQty(p._id,1)} style={{ width:26, height:26, borderRadius:7, border:"1.5px solid #22c55e", background:"#f0fdf4", fontSize:14, cursor:"pointer", fontWeight:700, color:"#166634" }}>+</button>
//                   </div>
//                   <button onClick={()=>removeFromCart(p._id)} style={{ background:"none", border:"none", color:"#ef4444", cursor:"pointer", padding:"4px", display:"flex", alignItems:"center" }}>
//                     <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6"/></svg>
//                   </button>
//                 </div>
//               ))}
