...
// // // // ─── Veggie nutrient database (% of daily value per 100g) ─────────────────────
// // // const VEGGIE_DB = {
// // //   "Spinach":      { emoji:"🥬", cal:23,  n:{ vitaminA:188, vitaminC:28,  iron:36, calcium:10, fiber:22, protein:12 }},
// // //   "Carrot":       { emoji:"🥕", cal:41,  n:{ vitaminA:334, vitaminC:10,  iron:3,  calcium:3,  fiber:28, protein:4  }},
// // //   "Broccoli":     { emoji:"🥦", cal:34,  n:{ vitaminA:12,  vitaminC:149, iron:5,  calcium:5,  fiber:21, protein:14 }},
...
// // // // ─── Gemini AI: nutrition advice ─────────────────────────────────────────────
// // // async function getGeminiNutritionAdvice(entries, nutrientTotals, score) {
// // //   const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
// // //   if (!GEMINI_API_KEY) throw new Error("GEMINI_API_KEY not set");

// // //   const loggedItems = entries.map(e => `${e.vegetable} (${e.grams}g)`).join(", ");
// // //   const deficiencies = Object.entries(nutrientTotals)
// // //     .filter(([, v]) => v < 40)
// // //     .map(([k]) => k)
// // //     .join(", ") || "none";

// // //   const prompt = `You are a professional nutritionist and dietitian specializing in Sri Lankan food habits.

// // // Today's food log: ${loggedItems || "nothing logged yet"}
// // // Current nutrient score: ${score}%
// // // Nutrient deficiencies (below 40% daily value): ${deficiencies}

// // // Respond with ONLY a valid JSON object. No markdown. No explanation. Just raw JSON:

// // // {
// // //   "summary": "2-sentence personal summary of today's nutrition",
// // //   "deficiencyAlert": "specific warning about the most critical deficiency, or null if all nutrients are adequate",
// // //   "recommendations": [
// // //     "Specific food recommendation 1 with reason",
// // //     "Specific food recommendation 2 with reason",
// // //     "Specific food recommendation 3 with reason"
// // //   ],
// // //   "tomorrowPlan": [
// // //     { "meal": "Breakfast", "suggestion": "specific Sri Lankan meal suggestion", "benefit": "why it helps" },
// // //     { "meal": "Lunch",     "suggestion": "specific Sri Lankan meal suggestion", "benefit": "why it helps" },
// // //     { "meal": "Dinner",    "suggestion": "specific Sri Lankan meal suggestion", "benefit": "why it helps" }
// // //   ],
// // //   "tipOfDay": "one practical, motivational nutrition tip tailored to what they logged"
// // // }

// // // Rules:
// // // - Be specific to Sri Lankan food culture (mention local foods like dhal curry, gotu kola, karapincha, etc.)
// // // - If nothing is logged, suggest starter foods to log
// // // - Keep each field concise (under 80 words)
...
// // //     // Compute current nutrient totals
// // //     const totals = {};
// // //     entries.forEach(e => {
// // //       e.nutrients.forEach((v, k) => { totals[k] = (totals[k] || 0) + v; });
// // //     });
...
// // //     console.log(`🤖 Requesting Gemini nutrition advice — score ${score}%, ${entries.length} entries`);

// // //     const advice = await getGeminiNutritionAdvice(entries, totals, score);
...
// // //       success: false,
// // //       advice: {
// // //         summary:          "Track your nutrition daily to see personalised AI insights.",
// // //         deficiencyAlert:  null,
// // //         recommendations:  [
// // //           "Add leafy greens like Kale or Spinach to boost Vitamin A and Iron.",
// // //           "Include Broccoli or Bell Pepper for a strong Vitamin C boost.",
// // //           "Try Drumstick (Moringa) — it is a powerful local superfood rich in calcium.",
// // //         ],
// // //         tomorrowPlan: [
// // //           { meal: "Breakfast", suggestion: "Oats with banana and milk",        benefit: "Provides fiber, calcium, and potassium" },
// // //           { meal: "Lunch",     suggestion: "Dhal curry with spinach and rice", benefit: "High in iron, protein, and Vitamin A" },
// // //           { meal: "Dinner",    suggestion: "Stir-fried vegetables with tofu",  benefit: "Balanced vitamins and plant protein" },
// // //         ],
// // //         tipOfDay: "Eating a rainbow of vegetables daily ensures you cover all essential vitamins and minerals!",
// // //       },
// // //     });
// // //   }
// // // };
...
// //     return res.json({ success: true, advice, model: "cerebras/llama-4-scout-17b" });
...
// //  Nutrition Tracker powered by Cerebras AI (llama-4-scout-17b)
// //
// //  Flow:
// //    POST /api/nutrition/add      → log a food entry, recalculate nutrient score
// //    GET  /api/nutrition/today    → get today's full log
// //    GET  /api/nutrition/weekly   → get last 7 days summary
// //    GET  /api/nutrition/veggies  → list all available foods in VEGGIE_DB
// //    GET  /api/nutrition/ai-advice → Cerebras generates personalized meal advice
// //    DELETE /api/nutrition/:entryId → remove an entry from today's log
// // ─────────────────────────────────────────────────────────────────────────────
...
// // ─── Nutrient database (% of daily value per 100g) ───────────────────────────
// // Covers vegetables, fruits, seafood, legumes, dairy, and staples common
// // in Sri Lankan diets. Extend this list as needed.
// const VEGGIE_DB = {
//   // Vegetables
//   "Spinach":       { emoji: "🥬", cal: 23,  n: { vitaminA: 188, vitaminC: 28,  iron: 36, calcium: 10, fiber: 22, protein: 12 }},
//   "Carrot":        { emoji: "🥕", cal: 41,  n: { vitaminA: 334, vitaminC: 10,  iron: 3,  calcium: 3,  fiber: 28, protein: 4  }},
...
// // ─── Nutrient score calculator ────────────────────────────────────────────────
// // Score = average of how close each key nutrient is to 100% of daily value
// // Capped per nutrient so you can't compensate one nutrient with excess of another
// function calcScore(totals) {
//   const keys = ["vitaminA", "vitaminC", "iron", "calcium", "fiber", "protein"];
//   const sum  = keys.reduce((s, k) => s + Math.min(totals[k] || 0, 100), 0);
//   return Math.round(sum / keys.length);
// }

// // ─── Cerebras Nutrition Advice ────────────────────────────────────────────────
// // Sends today's food log + nutrient status to Cerebras, gets back a personalized
// // Sri Lankan meal plan and deficiency alerts.
// async function getCerebrasNutritionAdvice(entries, nutrientTotals, score) {
//   const client = getCerebras();
...
//   const systemPrompt = `You are a professional clinical nutritionist and registered dietitian with deep expertise in Sri Lankan dietary habits, local foods, and tropical nutritional needs. Provide personalized, practical advice considering:
// - Locally available Sri Lankan foods (rice, dhal, coconut milk, gotu kola, karapincha, jak fruit, mukunuwenna, etc.)
// - Affordable, accessible meal options for everyday Sri Lankan families
// - Cultural food preferences and traditional cooking methods
// - Tropical climate nutritional requirements

// Always be warm, encouraging, and specific. Never give generic advice.`;

//   const userPrompt = `A consumer in Sri Lanka is using a nutrition tracking app. Here is their data for today:

// TODAY'S FOOD LOG: ${loggedItems}
// CURRENT NUTRIENT SCORE: ${score}% of daily goal
// DEFICIENT NUTRIENTS (below 40% daily value): ${deficientNutrients}
// ADEQUATE NUTRIENTS (above 70% daily value): ${adequateNutrients}
// TOTAL CALORIES TODAY: ${totalCalories} kcal

// Analyze their nutrition and respond with ONLY a valid JSON object — no markdown, no code fences, no extra text:

// {
//   "summary": "A warm, personalized 2-sentence summary of today's nutrition. Be specific about what they ate and how it helps.",
//   "deficiencyAlert": "Clear, specific warning about the most critical nutrient gap and its health consequences. Set to null if no deficiencies.",
//   "recommendations": [
//     "Actionable recommendation 1 — mention exact local Sri Lankan food with portion size",
//     "Actionable recommendation 2 — mention exact local Sri Lankan food with portion size",
//     "Actionable recommendation 3 — mention exact local Sri Lankan food with portion size"
//   ],
//   "tomorrowPlan": [
//     {
//       "meal": "Breakfast",
//       "suggestion": "Specific Sri Lankan breakfast with ingredients and rough quantities",
//       "benefit": "Which exact nutrients this provides and why the body needs them today"
//     },
//     {
//       "meal": "Lunch",
//       "suggestion": "Specific Sri Lankan lunch with ingredients",
...
//       "meal": "Dinner",
//       "suggestion": "Specific Sri Lankan dinner with ingredients",
//       "benefit": "How this completes today's nutritional goals"
//     }
//   ],
//   "tipOfDay": "One practical, motivating tip tailored to what they actually logged today — not generic advice."
// }

// RULES:
// - Every recommendation must name a specific Sri Lankan food (e.g., 'a bowl of dhal curry with mukunuwenna', 'gotu kola sambol', 'jak fruit curry')
// - tomorrowPlan suggestions must be complete meal descriptions, not just ingredient lists
// - If nothing was logged, encourage them to start tracking and suggest easy first foods
// - Keep each field under 100 words
// - Return ONLY the JSON object`;
...
//   if (!rawText) throw new Error("Cerebras returned an empty response");

//   const cleaned   = rawText.replace(/```json\s*/gi, "").replace(/```\s*/g, "").trim();
//   const jsonStart = cleaned.indexOf("{");
//   const jsonEnd   = cleaned.lastIndexOf("}");
//   if (jsonStart === -1 || jsonEnd === -1) throw new Error("No valid JSON in Cerebras response");
...
// // ── GET /api/nutrition/veggies ────────────────────────────────────────────────
// // Returns list of all foods in the database (name, emoji, calories per 100g)
// export const getVeggieList = async (req, res) => {
//   res.json(
//     Object.entries(VEGGIE_DB).map(([name, v]) => ({
//       name,
//       emoji: v.emoji,
//       cal:   v.cal,
//     }))
...
// // Returns today's nutrition log for the authenticated consumer
// export const getTodayLog = async (req, res) => {
//   try {
//     const today = new Date().toISOString().slice(0, 10);
//     const log   = await NutritionLog.findOne({ consumer: req.user._id, date: today });
...
// // Returns the last 7 days of logs (score + calories per day)
// export const getWeeklyLogs = async (req, res) => {
//   try {
//     const days = Array.from({ length: 7 }, (_, i) => {
//       const d = new Date();
...
//         return {
//           date,
//           nutrientScore:  l?.nutrientScore  || 0,
...
// // Body: { vegetable: string, grams: number }
// // Adds an entry to today's log and recalculates nutrient score
// export const addEntry = async (req, res) => {
//   try {
//     const { vegetable, grams } = req.body;
//     if (!vegetable || !grams) {
//       return res.status(400).json({ message: "vegetable and grams are required" });
//     }
...
//       calories:  Math.round(info.cal * ratio),
//       nutrients: new Map(
//         Object.entries(info.n).map(([k, v]) => [k, Math.round(v * ratio)])
//       ),
//     };
...
//     await log.save();
//     res.status(201).json({ message: "Entry added", log });
//   } catch (e) {
//     res.status(500).json({ message: e.message });
//   }
// };
...
// // Removes one entry from today's log and recalculates score
// export const removeEntry = async (req, res) => {
//   try {
//     const today = new Date().toISOString().slice(0, 10);
//     const log   = await NutritionLog.findOne({ consumer: req.user._id, date: today });
...
// // ── GET /api/nutrition/ai-advice ──────────────────────────────────────────────
// // Calls Cerebras to generate a personalized Sri Lankan meal plan + deficiency
// // alerts based on today's actual food log.
// export const getAiAdvice = async (req, res) => {
//   try {
...
//     console.log(`🧠 Requesting Cerebras nutrition advice — score: ${score}%, entries: ${entries.length}`);
...
//     // Graceful static fallback — never crash the consumer's page
//     return res.json({
//       success: false,
//       advice: {
//         summary:
//           "Track your meals daily to unlock personalised AI nutrition insights from Cerebras.",
//         deficiencyAlert: null,
//         recommendations: [
//           "Add a cup of kale or mukunuwenna for a powerful iron and Vitamin A boost.",
//           "Have a glass of fresh orange juice or bell peppers to meet your Vitamin C needs.",
//           "Include half a cup of dhal curry at lunch — one of Sri Lanka's best protein and iron sources.",
//         ],
//         tomorrowPlan: [
//           {
//             meal:       "Breakfast",
//             suggestion: "Oat porridge with sliced banana and a boiled egg",
//             benefit:    "Provides fiber, potassium, and complete protein to start the day",
//           },
//           {
//             meal:       "Lunch",
//             suggestion: "Red dhal curry with mukunuwenna and steamed red rice",
//             benefit:    "High in iron, Vitamin A, fiber, and plant protein — targets common deficiencies",
//           },
//           {
//             meal:       "Dinner",
//             suggestion: "Stir-fried vegetables (carrot, broccoli, bell pepper) with tofu",
//             benefit:    "Covers Vitamin A, C, calcium, and plant protein in one balanced meal",
//           },
//         ],
//         tipOfDay:
//           "Sri Lanka has some of the world's best superfoods — drumstick leaves (murunga), gotu kola, and jackfruit. Try adding at least one to your daily diet!",
//       },
//     });
//   }
// };
