...
// //  Crop Disease Analyzer powered by Cerebras AI (llama-4-scout-17b)
// //
// //  Flow:
// //    POST /api/disease/scan
// //      → multer saves image to disk
// //      → image buffer read → base64 encoded
// //      → Cerebras text model prompted with crop name + image metadata
// //      → structured JSON diagnosis returned & saved to MongoDB
// // ─────────────────────────────────────────────────────────────────────────────
...
// // ─── Fallback (returned when AI call fails) ───────────────────────────────────
// const FALLBACK = {
//   cropIdentified:     "Unknown",
//   diseaseName:        "Unable to identify",
//   scientificName:     "Unknown",
...
//   severity:           "Low",
//   isHealthy:          false,
//   symptoms:           "AI analysis unavailable — please retry or check CEREBRAS_API_KEY.",
//   detailedAnalysis:   "",
//   preventionTips:     "Ensure good air circulation, avoid overwatering, and remove visibly infected leaves.",
//   organicTreatments: [
//     { name: "Neem Oil Spray",       dosage: "5ml per liter",  instruction: "Spray all leaf surfaces every 5 days for 3 weeks." },
//     { name: "Baking Soda Solution", dosage: "5g per liter",   instruction: "Spray weekly to suppress fungal spread." },
//   ],
...
// // ─── Cerebras Disease Analysis ────────────────────────────────────────────────
// // Cerebras is a text-only model. We pass the crop name + image metadata and ask
// // the model to reason as a plant pathologist using its agricultural knowledge.
// // When Cerebras releases vision support, replace with image_url in the prompt.
// async function analyzeWithCerebras(imageBuffer, mimeType, cropName, farmerNotes = "") {
//   const client  = getCerebras();
//   const sizeKB  = Math.round(imageBuffer.length / 1024);
//   const format  = mimeType.split("/")[1]?.toUpperCase() || "JPEG";
...
// When given a crop name, diagnose the most statistically likely disease based on:
// - Most prevalent diseases for that crop in Sri Lanka and South Asia
// - Typical growth-stage problems (blight, wilt, rot, mite infestations, fungal/bacterial infections)
// - Symptoms Sri Lankan farmers most frequently report

// Provide specific, actionable advice using inputs (chemicals, organic materials) that Sri Lankan farmers can actually access locally.`;

//   const userPrompt = `A Sri Lankan farmer uploaded a crop photo and needs an expert plant disease diagnosis.

// CROP: ${cropName}
// IMAGE: ${format} file, ${sizeKB} KB
// FARMER NOTES: ${farmerNotes || "None provided"}

// Diagnose the most likely disease for this crop under typical Sri Lankan growing conditions.

// Respond with ONLY a valid JSON object — absolutely no markdown, no code fences, no explanation:

// {
//   "cropIdentified": "${cropName}",
//   "isHealthy": false,
//   "diseaseName": "most likely disease name",
//   "scientificName": "scientific name of causative pathogen",
//   "confidence": 82,
//   "severity": "Medium",
//   "symptoms": "Detailed description of visible symptoms a farmer would observe: leaf color changes, spots, lesions, wilting patterns, discoloration",
//   "detailedAnalysis": "Expert narrative explaining what the disease is, why it happens, key physiological impacts, and comprehensive management strategy. Format: 'The crop is affected by... What is happening is... Key points: ... Management: ... Important: ...' (150-200 words)",
//   "preventionTips": "3-4 specific prevention steps tailored to Sri Lankan farming practices",
//   "organicTreatments": [
//     {
//       "name": "Locally available organic treatment name",
//       "dosage": "Exact measurement e.g. 5ml per liter of water",
//       "instruction": "Step-by-step application instructions with timing and frequency"
//     },
//     {
...
// STRICT RULES:
// - isHealthy must be true or false (boolean)
// - If crop appears healthy: isHealthy=true, diseaseName="None", severity="None", treatments=[]
// - severity must be exactly one of: None, Low, Medium, High
// - confidence must be an integer between 60 and 95
// - Provide exactly 2 organic AND 2 chemical treatments when disease is found
// - organicTreatments and chemicalTreatments must be arrays (empty [] if healthy)
...
//   const rawText = response.choices?.[0]?.message?.content || "";
//   console.log("🧠 Cerebras disease raw response:", rawText.slice(0, 400));
...
//   if (!Array.isArray(parsed.organicTreatments))                      parsed.organicTreatments  = FALLBACK.organicTreatments;
//   if (!Array.isArray(parsed.chemicalTreatments))                     parsed.chemicalTreatments = FALLBACK.chemicalTreatments;
//   if (typeof parsed.confidence !== "number")                         parsed.confidence         = 75;
...
// // ── POST /api/disease/scan ────────────────────────────────────────────────────
// // Requires: multipart/form-data with fields: image (file), cropName (string)
// // Optional: notes (string) — farmer's own symptom description
// export const scanCrop = async (req, res) => {
//   try {
//     if (!process.env.CEREBRAS_API_KEY) {
//       return res.status(500).json({
//         message: "AI service not configured. Add CEREBRAS_API_KEY to your .env file.",
//       });
//     }
...
//     console.log(`🔬 Analyzing "${cropName}" — ${imageBuffer.length} bytes — ${mimeType}`);
...
//     try {
//       aiResult  = await analyzeWithCerebras(imageBuffer, mimeType, cropName, notes);
//       aiPowered = true;
//       console.log(`✅ Cerebras: "${aiResult.diseaseName}" — confidence ${aiResult.confidence}%`);
//     } catch (aiErr) {
//       console.error("❌ Cerebras analysis failed:", aiErr.message);
//       // Continues with FALLBACK below
//     }
...
//       message:  aiPowered
//         ? "✅ Cerebras AI analysis complete."
//         : "⚠ Fallback mode — Cerebras AI unavailable. Check your CEREBRAS_API_KEY.",
//       scan,
//       aiPowered,
//     });

//   } catch (err) {
//     console.error("scanCrop error:", err.message);
//     return res.status(500).json({ message: err.message });
//   }
// };
...
//     // Remove associated image file from disk
//     if (scan.imageUrl) {
...
// // ── GET /api/disease/admin/all ────────────────────────────────────────────────
// export const adminGetAllScans = async (req, res) => {
//   try {
//     const scans = await Disease
//       .find()
//       .populate("farmer", "name email")
//       .sort("-createdAt");
//     res.json({ count: scans.length, scans });
