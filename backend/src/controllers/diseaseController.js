...
// // ── Gemini Vision API ──────────────────────────────────────
// async function analyzeWithGemini(imageBuffer, mimeType, cropName) {
...
// Look at the image and identify any diseases, infections, or health issues visible.
...
//   "isHealthy": false,
//   "diseaseName": "name of the disease you identified",
//   "scientificName": "scientific name of the pathogen",
//   "confidence": 87,
//   "severity": "Medium",
//   "symptoms": "describe exactly what you see: colors, spots, patterns, texture changes",
//   "organicTreatments": [
//     { "name": "Neem Oil Spray", "dosage": "5ml per liter of water", "instruction": "Spray on all leaf surfaces every 5 days for 3 weeks" },
//     { "name": "Baking Soda Solution", "dosage": "5g per liter", "instruction": "Spray weekly to prevent fungal spread" }
//   ],
//   "chemicalTreatments": [
...
// - If multiple diseases visible, report the most severe one
// - confidence should be between 60-99
...
//     if (!GEMINI_API_KEY)    return res.status(500).json({ message: "AI service not configured. Add GEMINI_API_KEY to .env" });

//     const cropName = req.body.cropName.trim();
//     const imageUrl = `/uploads/${req.file.filename}`;
//     const mimeType = req.file.mimetype || "image/jpeg";

//     // ── Log req.file details to debug path issues ──────────
//     console.log("📋 req.file:", {
//       filename: req.file.filename,
//       path:     req.file.path,
//       size:     req.file.size,
//       mimetype: req.file.mimetype,
//       buffer:   req.file.buffer ? "EXISTS" : "UNDEFINED (diskStorage)",
//     });

//     // ── Get image buffer ───────────────────────────────────
//     // diskStorage → use req.file.path (multer sets this automatically)
//     // memoryStorage → use req.file.buffer
//     let finalBuffer;
...
//       // diskStorage — req.file.path is the full absolute path set by multer
//       console.log(`📁 Reading from disk: ${req.file.path}`);
//       if (!fs.existsSync(req.file.path)) {
//         return res.status(500).json({ message: `Image file not found at: ${req.file.path}` });
//       }
...
//     try {
//       aiResult  = await analyzeWithGemini(finalBuffer, mimeType, cropName);
//       aiPowered = true;
//       console.log(`✅ Gemini: ${aiResult.diseaseName} (${aiResult.confidence}%)`);
//     } catch (geminiErr) {
...
//     res.status(201).json({
//       message:   aiPowered ? "✅ Gemini AI analysis complete." : "⚠ Fallback mode — Gemini unavailable.",
//       scan,
//       aiPowered,
//     });
